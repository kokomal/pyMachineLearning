    ...
    while (iter < maxIter):  # 最大循环次数为maxIter
        ...
        for i in range(m):
            ...
            if ((labelMat[i] * Ei < -toler) and (alphas[i] < C)) or ((labelMat[i] * Ei > toler) and (alphas[i] > 0)):
                ...
                if (labelMat[i] != labelMat[j]):  # y1与y2不同号，αi*yi + αj*yj = ς ---> α1-α2 = k
                    L = max(0, alphas[j] - alphas[i])
                    H = min(C, C + alphas[j] - alphas[i])
                else:
                    L = max(0, alphas[j] + alphas[i] - C)
                    H = min(C, alphas[j] + alphas[i])
                if L == H: print("L==H"); continue  # L==H继续 
                #  eta = 2XiXjT - XiXiT - XjXjT
                eta = 2.0 * dataMatrix[i, :] * dataMatrix[j, :].T - dataMatrix[i, :] * dataMatrix[i, :].T - dataMatrix[j, :] * dataMatrix[j, :].T
                if eta >= 0: print("eta>=0"); continue  # eta>=0，继续
                alphas[j] -= labelMat[j] * (Ei - Ej) / eta  # α2unclip = α2old + y2(E1-E2)/(K11+K22-2K12)
                alphas[j] = clipAlpha(alphas[j], H, L)
                if (abs(alphas[j] - alphaJold) < 0.00001): print("j not moving enough"); continue  # α变化低于预期也继续
...
class optStructKernel:
    ...
    def __init__(self, dataMatIn, classLabels, C, toler, kTup):  # Initialize the structure with the parameters 
        ...
        for i in range(self.m):
            self.K[:, i] = kernelTrans(self.X, self.X[i, :], kTup)  # 核函数
...
def updateEkKernel(oS, k):  # after any alpha has changed update the new value in the cache
    Ek = calcEkKernel(oS, k)
    oS.eCache[k] = [1, Ek]
...
def innerLKernel(i, oS):
    ...
    if ((oS.labelMat[i] * Ei < -oS.tol) and (oS.alphas[i] < oS.C)) or ((oS.labelMat[i] * Ei > oS.tol) and (oS.alphas[i] > 0)):
        j, Ej = selectJKernel(i, oS, Ei)  # this has been changed from selectJrand
        ...
        updateEkKernel(oS, j)  # added this for the Ecache
        if (abs(oS.alphas[j] - alphaJold) < 0.00001): print("j not moving enough"); return 0
        oS.alphas[i] += oS.labelMat[j] * oS.labelMat[i] * (alphaJold - oS.alphas[j])  # update i by the same amount as j
        updateEkKernel(oS, i)  # added this for the Ecache                    #the update is in the oppostie direction
